...
@else
    ...
    @if(auth()->guard('admin')->check() && auth()->guard('admin')->user()->canManageHessamCMSPosts())
        ...
        <div class='alert alert-danger'>Custom blog post blade view file
                    ...
                    href='https://hessam.binshops.com/laravel/packages/help-documentation/laravel-blog-package-hessamcms'
                    target='_blank'>See Laravel Blog Package help here</a>.
                    </div>

    @else
        {{--is not logged in, or User::canManageHessamCMSPosts() for current user == false--}}

...
<p class="blog_body_content">
    ...
    {{--@if(config("hessamcms.use_custom_view_files")  && $post->use_view_file)--}}
    {{--                                // use a custom blade file for the output of those blog post--}}
    {{--   @include("hessamcms::partials.use_view_file")--}}
    {{--@else--}}
...
@includeWhen($post->author,"hessamcms::partials.author",['post'=>$post])
@includeWhen($post->categories,"hessamcms::partials.categories",['post'=>$post])
